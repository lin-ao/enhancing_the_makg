...
entity_dict = {}
relations_dict = {}
with open("02.papers_input.txt", "r") as f:
    for line in f:
        print("Dictionary: " + str(line_count))
        if len(line.split(" ")) > 1:
            sub = line.split(" ")[0].rstrip(">").replace("<http://ma-graph.org/", "")
            pred = line.split(" ")[1].rstrip(">").split("/")[-1].replace("22-rdf-syntax-ns#", "")
            obj = " ".join(line.split(" ")[2:]).strip().rstrip(".").strip().replace("^^<http://www.w3.org/2001/XMLSchema#date>", "").replace("^^<http://www.w3.org/2001/XMLSchema#integer>", "").replace("^^<http://www.w3.org/2001/XMLSchema#string>", "").rstrip(">").replace("<http://ma-graph.org/", "").replace("<http://purl.org/spar/fabio/", "").replace('"', '')
...
with open("03.paper_entities.dict", "w") as f:
    for item in entity_dict:
        f.write(str(entity_dict[item]) + "\t" + str(item) + "\n")
with open("03.paper_relations.dict", "w") as f:
    for item in relations_dict:
        f.write(str(relations_dict[item]) + "\t" + str(item) + "\n")

line_count = 1
with open("02.papers_input.txt", "r") as f:
    with open("03.paper_train.tsv", "w") as g:
        with open("03.paper_valid.tsv", "w") as h:
            with open("03.paper_test.tsv", "w") as i:
                for line in f:
